...
class ldamodel_sent():
    ...
    def initial(self):
        ...
        self.gamma_dl = np.full((self.D, self.S), fill_value=0.0)
        ...
        print('开始赋予sentiment')
        alpha_prior=[1/self.T for _ in range(self.T)]
        gamma_prior=[1/self.S for _ in range(self.S)]
        for i, doc in enumerate(self.corpus):
            sentimentDistribution = self.sampleFromDirichlet(gamma_prior)
            topicDistribution = np.zeros(( self.S,self.T))
            for s in range(self.S):
                topicDistribution[s, :] = self.sampleFromDirichlet(alpha_prior)
            for j, word in enumerate(doc): 
                ...
                if word in self.sentiment_dict.keys():
                    sentiment=self.sentiment_dict[word]
                else:
            ...
            for i, doc in enumerate(self.corpus):
                ...
                for j, word in enumerate(doc):
                    try:
                        ...
                        probabilities_ts= self.resampling(i, word_id)
        ...
        for i, doc in enumerate(self.corpus):
            ...
            for word in sent.keys():
                ...
                ui = (sum([model_cxl.similarity(word, oo) for oo in hownet_word_good]) / len(hownet_word_good) -
                      sum([model_cxl.similarity(word, oo) for oo in hownet_word_bad]) / len(hownet_word_bad)) * \
                     sent[word]
...
if __name__ == '__main__':
    path1='C:/Users/Administrator/Desktop/data/corpus/handle_corpus_zhijie.txt'
    print('开始加载数据')
...
##    
    sentiment_path='C:/Users/Administrator/Desktop/data/corpus/sentiment_dict_goujian_zhijie.txt'
...
##            corpus_cup.append(doc)
    ...
    for nums in topic_nums:
        P = ldamodel_sent(nums, 4, 0.01, 0.01, 0.01, comment_train, len(comment_train), 10, sentiment_dict, label_total)
        P.createdictionary()
        print('开始初始化')
        P.initial()
        ...
        start=time.time()
        P.gibbssampling()
        score, recall, f1_score = P.getzhibiao()
